...
namespace core {
    ...
    void make_decision_by(human* h) {
        if (h->should_eat() && h->curr_state() != state_type::eating) {
            int travel_time = timer_manager::instance()->clock() + graph::instance()->distance(h->m_location, loc_type::restaurant);
            ...
            int travel_time = timer_manager::instance()->clock() + graph::instance()->distance(h->m_location, loc_type::bar);
            ...
            int travel_time = timer_manager::instance()->clock() + graph::instance()->distance(h->m_location, loc_type::party);
            ...
            int travel_time = timer_manager::instance()->clock() + graph::instance()->distance(h->m_location, loc_type::mall);
    ...
    void process_message(human* h, const message& msg) {
        ...
        int travel_time = timer_manager::instance()->clock() + graph::instance()->distance(h->m_location, dest);
