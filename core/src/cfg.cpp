    ...
    game_config::game_config(const nlohmann::json& doc) {
...

        {
            ...
            for (auto it = types.begin(); it != types.end(); ++it) {
                tile_type type = util::char_to_tile(it.key()[0]);
                auto& data = it.value();
...

        {
            ...
            for (auto it = types.begin(); it != types.end(); ++it) {
                ...
                auto& data = it.value();
                building_cfg[type] = {t, type, data.at("builders").get<int>(), {data.at("resources").at("wood").get<int>(), data.at("resources").at("coal").get<int>()}, data.at("time").get<int>()};
            }
        }

        {
            ...
            for (auto it = types.begin(); it != types.end(); ++it) {
                ...
                auto& data = it.value();
                resource_cfg[type] = {type, {data.at("in").at("wood").get<int>(), data.at("in").at("coal").get<int>()}, data.at("out").get<int>(), data.at("time").get<int>()};
            }
        }

        {
            const auto& types = doc.at("units");
            for (auto it = types.begin(); it != types.end(); ++it) {
                unit_type type = util::str_to_unit(it.key());
                auto& data = it.value();
                unit_cfg[type] = {type, util::str_to_unit(data.at("from").get<std::string>()), data.at("carry").get<int>(), data.at("upgrade_time").get<int>()};
            }
        }
    }
